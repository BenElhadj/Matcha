...
const unreportUser = async (req, res) => {
	...
	// Accept both 'id' and 'block_row_id' for compatibility
	const block_row_id = req.body.block_row_id || req.body.id;
	if (!block_row_id || isNaN(block_row_id)) {
		return res.json({ status: 'error', type: 'report', message: 'Invalid request', data: null });
	}
	try {
		await userModel.unreportUser(req.user.id, block_row_id);
		// Vérification de la suppression
		const check = await require('../utility/startbdd').pool.query(
			'SELECT * FROM blocked WHERE id = $1 AND blocker = $2 AND type = $3',
			[block_row_id, req.user.id, 'report']
		);

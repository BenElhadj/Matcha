...
<template>
  <q-layout class="justify-center q-py-none">
    <div
      class="col-xs-12 col-sm-11 col-md-9 offset-lg-3 offset-xl-4 offset-md-2 offset-sm-1 badge text-capitalize"
    >
      <h2 class="q-mb-md">
        <div class="text-truncate full_name q-pr-md inline-block">
        ...
        <q-tooltip v-if="!settings && like" anchor="bottom middle" self="top middle">
          <template v-slot:activator="{ on }">
            <span x="0px" y="0px" viewBox="0 0 407.585 407.585" v-on="on" class="match_icon">
              <q-icon
                :name="match ? 'mdi-heart-multiple' : 'mdi-heart-multiple-outline'"
                color="red"
              ></q-icon>
            </span>
          </template>

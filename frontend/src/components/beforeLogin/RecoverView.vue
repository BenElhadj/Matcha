        ...
        <div class="my-4">
          ...
          <q-form @submit.prevent="submit" class="my-4">
            ...
            >
              <template #append>
                <q-icon
                  :name="showConfPass ? 'mdi-eye-off' : 'mdi-eye'"
                  class="cursor-pointer"
...
const passRules = [
  ...
  (v) =>
    /^(?=.*\d)(?=.*[A-Za-z])[0-9A-Za-z!@#$%]+$/.test(v) ||
    'Password must contain at least one letter, one number and one special char',
...
const valid = computed(() => {
  return (
    passRules.every((rule) => rule(password.value) === true) &&
...
const beforeDestroy = async () => {
  ...
  const url = `${API_URL}/api/auth/destroykey`
